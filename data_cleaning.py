...
def preprocess_column(col):
     ...
     # Handle None/NaN gracefully early
     if col == 'NAN' or col is None:
         return None # Or return an empty string '', depending on desired handling

     # Standardize variations of Non-Graded first
     col = re.sub(r'NON\s*[-–]?\s*GRADED?', 'NG', col, flags=re.IGNORECASE)
     col = re.sub(r'\bN[\s\.]?G[\s\.]?\b', 'NG', col, flags=re.IGNORECASE) # Capture N G, N.G., NG. etc.

     # Ensure NG is correctly associated
     col = re.sub(r'\b(JHS|JUNIOR HIGH SCHOOL)\b(?!\s*NG)', r'\1 NG', col, flags=re.IGNORECASE)
     col = re.sub(r'\b(ES|ELEM|ELEMENTARY)\b(?!\s*NG)', r'\1 NG', col, flags=re.IGNORECASE)

     # Standardize Grade Levels and Kindergarten
     col = re.sub(r'KINDERGARTEN', 'K', col, flags=re.IGNORECASE)
     col = re.sub(r'GRADE\s*(\d{1,2})', r'G\1', col, flags=re.IGNORECASE) # G1, G10 etc.

     # Standardize Tracks/Strands - Be specific
     col = re.sub(r'\bARTS\s*&\s*DESIGN\b', 'ARTS', col, flags=re.IGNORECASE)
     col = re.sub(r'\bACADEMIC TRACK\b', 'ACAD', col, flags=re.IGNORECASE) # Make ACAD explicit if needed
     # Handle common ACAD strands explicitly adding ACAD prefix if missing near G11/G12
     col = re.sub(r'(G11|G12)\s+(ABM|HUMSS|STEM|GAS|PBM)\b', r'\1 ACAD - \2', col, flags=re.IGNORECASE)
     # Handle TVL, SPORTS, ARTS explicitly if missing near G11/G12
     col = re.sub(r'(G11|G12)\s+TVL\b', r'\1 TVL', col, flags=re.IGNORECASE) # Already has grade prefix usually
     col = re.sub(r'(G11|G12)\s+SPORTS\b', r'\1 SPORTS', col, flags=re.IGNORECASE)
     col = re.sub(r'(G11|G12)\s+ARTS\b', r'\1 ARTS', col, flags=re.IGNORECASE)

     # General cleanup
     col = col.replace('-', ' - ') # Standardize spacing around hyphens
     col = re.sub(r'\s{2,}', ' ', col).strip() # Remove extra spaces
...
def standardize_column_name(col):
    ...
    if processed_col is None: # Handle case where preprocess returns None
        return None # Or some default/indicator
    ...
    match = get_close_matches(final_col, standard_columns, n=1, cutoff=0.85) # Use a reasonable cutoff
...
def clean_data(file_path):
    ...
    if is_cleaned and df_check is not None:
        ...
        if is_school_level:
            ...
            # --- REGIONAL/OTHER LEVEL FILE CLEANING ---
...
# ... (keep standard_columns, region_mapping, standardize_region_values, preprocess_column, standardize_column_name) ...

standard_columns = [
    # ... (your full list of standard_columns) ...
    "K Male", "K Female", "G1 Male", "G1 Female", "G2 Male", "G2 Female", "G3 Male", "G3 Female",
    "G4 Male", "G4 Female", "G5 Male", "G5 Female", "G6 Male", "G6 Female",
    "Elem NG Male", "Elem NG Female", "G7 Male", "G7 Female", "G8 Male", "G8 Female",
    "G9 Male", "G9 Female", "G10 Male", "G10 Female", "JHS NG Male", "JHS NG Female",
...
# Example Usage (assuming we have a file 'my_regional_data.csv'):
# try:
